// ==========================================
// admin-kas.js - KAS System UI (CLEAN VERSION)
// ==========================================

console.log("🚀 admin-kas.js loaded");

// ==========================================
// HELPER FUNCTIONS (1 KALI DEKLARASI)
// ==========================================
...
function renderKasUI() {
    ...
    // Build bendahara options (exclude current admin)
...
function attachKasEventListeners() {
    ...
    document.querySelectorAll(".kas-mode-btn").forEach(btn => {
        btn.onclick = () => {
            ...
            const formSetoran = document.getElementById("kas-form-setoran");
            const formTransfer = document.getElementById("kas-form-transfer");
            if (formSetoran) formSetoran.style.display = currentMode === "setoran" ? "block" : "none";
            if (formTransfer) formTransfer.style.display = currentMode === "transfer" ? "block" : "none";
        };
    });
...
async function submitSetoran() {
    ...
    if (!spina || isNaN(spina)) { showToast("Spina harus diisi", true); return; }
...
async function submitTransfer() {
    ...
    if (!amount || isNaN(amount) || amount <= 0) { showToast("Jumlah harus diisi", true); return; }
...
console.log("✅ admin-kas.js selesai dimuat");
